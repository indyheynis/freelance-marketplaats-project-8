<x-base-layout>
    <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="mb-8">
            ...
            <h1 class="text-3xl font-bold text-slate-800 dark:text-slate-100">Create New Category</h1>
            <p class="text-slate-500 dark:text-slate-400 mt-1">Add a new category for commissions</p>
        </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm p-6">
            <form action="{{ route('categories.store') }}" method="POST" class="space-y-5">
                ...
                <div class="flex items-center gap-3 pt-4">
                    ...
                    <a href="{{ route('categories.index') }}" class="inline-flex items-center gap-2 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-200 px-5 py-2.5 rounded-lg font-medium transition-colors">
                        Cancel
                    </a>
                </div>
            </form>
