<x-base-layout>
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="flex justify-between items-center mb-8">
            <div>
                <h1 class="text-3xl font-bold text-slate-800">{{ __('My Applications') }}</h1>
                <p class="text-slate-500 mt-1">{{ __('Overview of all your applications') }}</p>
            </div>
        ...
        <div class="mb-6">
            <form method="GET" action="{{ route('applications.index') }}" class="flex items-center gap-3">
                <select name="status" onchange="this.form.submit()"
                    class="border border-slate-300 rounded-lg px-4 py-2 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-white">
                ...
                @if(request('status'))
                <a href="{{ route('applications.index') }}" class="text-sm text-slate-500 hover:text-slate-700 transition-colors">
                    {{ __('Clear filter') }}
                </a>
                @endif
            </form>
        ...
        <div class="space-y-4">
            ...
            <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
                ...
                <div class="flex items-center gap-4 text-sm text-slate-500 mb-3">
                    ...
                    <span>{{ __('Applied:') }} <span class="font-medium text-slate-700">{{ $application->created_at?->diffForHumans() ?? __('Unknown') }}</span></span>
                </div>

                @if($application->message)
                    <div class="bg-slate-50 rounded-lg p-3 mb-4 text-sm text-slate-600">
                        <span class="font-medium text-slate-700">{{ __('Your message:') }}</span> {{ $application->message }}
                    </div>
                @endif
            ...
            <div class="text-center py-16">
                ...
                <h3 class="text-lg font-medium text-slate-700 mb-2">{{ __('No applications yet') }}</h3>
                <p class="text-slate-500 mb-6">{{ __('Search for commissions that match your profile.') }}</p>
