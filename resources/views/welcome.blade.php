<x-base-layout>
    ...
    <section class="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 text-center">
            <span class="inline-block bg-indigo-100 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-200 text-sm font-semibold px-4 py-1.5 rounded-full mb-6">
                🚀 {{ __('The freelance marketplace for everyone') }}
            </span>
            <h1 class="text-5xl font-bold text-slate-900 dark:text-slate-100 mb-6 leading-tight">
                {!! __('Connect with top <span class="text-transparent bg-clip-text bg-gradient-to-r from-indigo-500 to-purple-600">freelancers</span><br>or find your next client') !!}
            </h1>
            <p class="text-xl text-slate-500 dark:text-slate-400 mb-10 max-w-2xl mx-auto">
                {{ __('FreelanceHub brings clients and freelancers together. Post commissions, browse opportunities, and get work done — all in one place.') }}
            </p>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid grid-cols-3 gap-8 text-center">
                <div>
                    ...
                    <div class="text-indigo-200 text-sm">{{ __('Active Freelancers') }}</div>
                </div>
                <div>
                    <div class="text-4xl font-bold mb-1">1.2k</div>
                    <div class="text-indigo-200 text-sm">{{ __('Commissions Posted') }}</div>
                </div>
                <div>
                    <div class="text-4xl font-bold mb-1">98%</div>
                    <div class="text-indigo-200 text-sm">{{ __('Satisfaction Rate') }}</div>
                </div>
            </div>
        </div>
    </section>
    ...
    <section class="py-20 bg-slate-50 dark:bg-slate-950">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-14">
                <h2 class="text-3xl font-bold text-slate-900 dark:text-slate-100 mb-3">{{ __('How it works') }}</h2>
                <p class="text-slate-500 dark:text-slate-400 text-lg">{{ __('Simple steps to get started') }}</p>
            </div>
            <div class="grid md:grid-cols-3 gap-8">
                <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm p-8 text-center">
                    ...
                    <h3 class="text-lg font-semibold text-slate-800 dark:text-slate-100 mb-2">{{ __('1. Create an account') }}</h3>
                    <p class="text-slate-500 dark:text-slate-400 text-sm">{{ __('Sign up as a freelancer or client and set up your profile in minutes.') }}</p>
                </div>
                <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm p-8 text-center">
                    ...
                    <h3 class="text-lg font-semibold text-slate-800 dark:text-slate-100 mb-2">{{ __('2. Post or browse') }}</h3>
                    <p class="text-slate-500 dark:text-slate-400 text-sm">{{ __('Clients post commissions, freelancers browse and find the perfect project.') }}</p>
                </div>
                <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm p-8 text-center">
                    ...
                    <h3 class="text-lg font-semibold text-slate-800 dark:text-slate-100 mb-2">{{ __('3. Get it done') }}</h3>
                    <p class="text-slate-500 dark:text-slate-400 text-sm">{{ __('Connect, collaborate and complete projects with ease.') }}</p>
                </div>
            </div>
        </div>
    </section>
    ...
    <section class="py-20 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-700">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-14">
                <h2 class="text-3xl font-bold text-slate-900 dark:text-slate-100 mb-3">{{ __('Built for both sides') }}</h2>
                <p class="text-slate-500 dark:text-slate-400 text-lg">{{ __("Whether you're hiring or looking for work, FreelanceHub has you covered.") }}</p>
            </div>
            <div class="grid md:grid-cols-2 gap-8">
                <div class="bg-gradient-to-br from-indigo-50 to-indigo-100 dark:from-indigo-950 dark:to-slate-900 rounded-xl border border-indigo-200 dark:border-indigo-800 p-8">
                    ...
                    <h3 class="text-xl font-bold text-slate-800 dark:text-slate-100 mb-3">{{ __('For Clients') }}</h3>
                    <ul class="space-y-2 text-slate-600 dark:text-slate-300 text-sm mb-6">
                        <li class="flex items-center gap-2">
                            ...
                            {{ __('Post commissions easily') }}
                        </li>
                        <li class="flex items-center gap-2">
                            <svg class="w-4 h-4 text-indigo-500 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" /></svg>
                            {{ __('Set your own budget & deadline') }}
                        </li>
                        <li class="flex items-center gap-2">
                            <svg class="w-4 h-4 text-indigo-500 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" /></svg>
                            {{ __('Find the right freelancer fast') }}
                        </li>
                    </ul>
